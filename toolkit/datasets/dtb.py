...
def ca():
    ...
    for jj in range(a):
        imgs=path+'/data_seq/'+str(name_list[jj])+'/img'
        txt=path+'/data_seq/'+str(name_list[jj])+'/groundtruth_rect.txt'
        att_txt=path+'/anno/att/'+str(name_list[jj])+'.txt'
        bbox=[]
        f = open(txt)               # 返回一个文件对象
        ...
        for ii in range(len(file)):
            li[ii]='data_seq/'+name_list[jj]+'/img/'+li[ii]
