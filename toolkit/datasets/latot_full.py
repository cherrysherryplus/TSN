...
def loaddata():
    ...
    test_txt = 'test_datasets_demo/LaTOT/dataset-split/TOT.txt'
    ...
    for jj in range(len(name_list)):
        ...
        for ii in range(len(file)):
            ...
            #     print(name_list[jj], 'annos', len(file), 'imgs', len(li))
            #     err_seqs.add(name_list[jj])
            line = file[ii].strip('\n').split(',')
...
class LaTOTfullVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        super(LaTOTfullVideo, self).__init__(name, root, video_dir,
                init_rect, img_names, gt_rect, attr, load_img)
        # self.absent = np.array(absent, np.int8)

class LaTOTfullDataset(Dataset):
    """
    Args:
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(LaTOTfullDataset, self).__init__(name, dataset_root)
        dataset_root = './test_datasets_demo/LaTOT'
        meta_data = loaddata()
        ...
        for video in pbar:
            ...
            self.videos[video] = LaTOTfullVideo(video,
                                          dataset_root,
