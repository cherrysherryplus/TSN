...
class Video(object):
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            ...
            print("traj_file = {}".format(traj_file))
    ...
    # def __iter__(self):
    #     min_length = min(len(self.img_names),len(self.gt_traj)) - 2
    #     for i in range(min_length):
    #         if self.imgs is not None:
    #             yield self.imgs[i], self.gt_traj[i]
    #         else:
    #             #print('=====>  i = {}  len(img_names) = {}    len(gt_traj) = {}'.format(i,len(self.img_names),len(self.gt_traj)))
    #             yield cv2.imread(self.img_names[i]), self.gt_traj[i]
